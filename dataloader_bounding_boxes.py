...
#         self.images = sorted(list(self.root.glob(f"{car}/{split}/RGB_wholeImage/*.png")))
        ...
        # by Ilan Figueirêdo
        elif self.nbr_classes == 1:
            # we do want to use only child seat data for training 
            self.images = [element for element in self.images if "_2" in str(element) or "_6" in str(element)]
            self.boxes = [element for element in self.boxes if "_2" in str(element) or "_6" in str(element)]
...
#     ax.imshow(image, vmin=0, vmax=1, cmap=None)
    ax.grid(False)
